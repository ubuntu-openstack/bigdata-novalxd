...
juju run --unit spark/0 "su hdfs -c 'time /usr/bin/spark-submit --class MainRun /tmp/anomaly-detection_2.10-1.0.jar --master spark://127.0.0.1 > /tmp/spark_job_results.txt'"
echo get results
